...
Exercise:
  ...
  description: Azure Application Gateway を作成して構成します。
  duration: 25 minutes
  level: 300
  islab: true
  primarytopics:
...
1. アプリケーション ゲートウェイのページで、**[+ 作成]** を選択し、ドロップダウン メニューから **[Application Gateway]** を選択します。
...
1. **[設定]**、**[サブネット]** の順に選択して、**BackendSubnet** を作成します。 **[+ サブネット]** を選択し、下の設定を入力してから **[追加]** を選択します。
...
1. 次の ARM テンプレートをデプロイして、この演習に必要な VM を作成します。
   ...
   >**注**: 既定の VM サイズの **NotAvailableForSubscription** でデプロイが失敗した場合、**[backend.parameters.json]** を開き、**vmSize** 値をサブスクリプションとリージョンで利用可能な VM サイズに更新し、デプロイ コマンドを再度実行してください。
