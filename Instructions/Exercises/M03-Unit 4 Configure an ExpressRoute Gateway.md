...
Exercise:
  ...
  description: ExpressRoute ゲートウェイを構成します。
  duration: 60 minutes
  level: 300
  islab: true
  primarytopics:
...
お使いの Azure 仮想ネットワークとオンプレミス ネットワークを ExpressRoute 経由で接続するには、最初に仮想ネットワーク ゲートウェイを作成する必要があります。 仮想ネットワーク ゲートウェイには 2 つの目的があります。ネットワーク間で IP ルートを交換することと、ネットワーク トラフィックをルーティングすることです。 プライベート接続でネットワーク トラフィックを送信するには、ゲートウェイの種類として "ExpressRoute" を使用します。 これは ExpressRoute ゲートウェイとも呼ばれ、ExpressRoute の構成時に使用するゲートウェイの種類です。


![仮想ネットワーク ゲートウェイの図。](../media/4-exercise-configure-expressroute-gateway.png)


## 推定時間: 60 分 (最大 45 分の展開待機時間を含む)
...
1. **[次へ: セキュリティ]** を選択し、変更は行わずに選択肢をレビューしてください。

1. **[次へ: アドレス空間]** を選択します。

1. **[IPv4 アドレス空間]** タブの **[アドレス空間]** タブに、「10.20.0.0/16」と入力します。

1. **既定の**サブネットを削除します。

1. **[+ サブネットの追加]** を選択します。 次の表の情報を使用して、サブネットを作成します。
...
1. Azure portal ページの「**リソース、サービス、ドキュメントの検索 (G+/)** 」で、仮想ネットワークと入力し、結果から「**仮想ネットワーク ゲートウェイ**」を選択します。 ページは **[ハイブリッド接続]** ハブの **[ExpressRoute ゲートウェイ]** ビューで開きます。

1. **[ExpressRoute ゲートウェイ]** ビューで **[+ 作成]** を選択します。
