...
Exercise:
  ...
  description: Traffic Manager プロファイルを作成して構成します。
  duration: 35 minutes
  level: 300
  islab: true
  primarytopics:
...
次に、エンドポイントの優先度に基づいて Traffic Manager プロファイルを作成します。 このプロファイルにより、Web アプリケーションが実行されているプライマリ サイトにユーザー トラフィックが誘導されます。 Traffic Manager では Web アプリケーションを継続的に監視し、米国東部のプライマリ サイトが使用できない場合は、西ヨーロッパのバックアップ サイトに自動的にフェールオーバーします。
...
1. Azure portal のホーム ページで、**[App Services]** を検索して選択します。 **[作成]** を選択し、**[Web アプリ]** を選択します。
...
1. **[Web アプリの作成]** ページの **[基本]** タブで、次の表の情報を使用して最初の Web アプリケーションを作成します。
   ...
   | 料金プラン     | **Premium V3 P1V3** (使用できない場合は別のプランを選択します)          |

   >**注:** **[米国東部]** がクォータ、容量、またはポリシーの制約により利用できない場合は、サブスクリプション内で利用可能な他のリージョンを選択してください。
    
1. **[モニター + セキュリティ保護]** タブを選択します。

1. **[Application Insights を有効にする]** で **[いいえ]** オプションを選択します。
...
1. Web ブラウザーのタブを開き、**DNS 名**のエントリ (contoso-tmprofile.trafficmanager.net) をアドレス バーに貼り付け (または入力して)、Enter キーを押します。
    ...
    >**注:** Web サイトが安全ではないというエラーが出た場合は、InPrivate ブラウズ ウィンドウを使用してください。 
...
1. **[Contoso-TMProfile]** ページの概要画面で、**[エンドポイント:]** の右にある数字 **2** を選びます。

1. **myPrimaryEndpoint** の [編集] ボタン (鉛筆アイコン) を選択します。

1. **myPrimaryEndpoint** ブレードの **[状態]** で、**[エンドポイントの有効化]** のチェック ボックスをオフにして、**[保存]** を選びます。

1. 必要に応じて、**myPrimaryEndpoint** ブレードを閉じます (ページの右上隅にある **[X]** を選択します)。
