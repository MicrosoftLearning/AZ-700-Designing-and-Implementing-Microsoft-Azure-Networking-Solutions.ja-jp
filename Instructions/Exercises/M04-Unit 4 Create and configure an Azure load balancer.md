...
Exercise:
  ...
  description: 内部ロード バランサーを作成して構成します。
  duration: 60 minutes
  level: 300
  islab: true
  primarytopics:
...
この演習では、架空の組織 Contoso Ltd. 用の内部ロード バランサーを作成します。 内部ロード バランサーを使用する場合、フロントエンドは仮想ネットワーク内にプライベート IP アドレスを持っています。 同じネットワーク内のホストからの接続をテストします。
...
1. **[次へ]** を選択します ([セキュリティ] タブが表示されます)。

1. **[Azure Bastion]** で、**[Azure Bastion を有効にする]** を選択し、以下の表の情報を入力します。
...
1. **[次へ]** を選択します ([IP アドレス空間] タブが表示されます)。

1. **[IP アドレス空間]** タブの **[IPv4 アドレス空間]** ボックスで、事前設定された IPv4 アドレス空間を **10.1.0.0/16** に置き換えます。

1. **[IP アドレス空間]** タブの **[サブネット]** で、**既定**のサブネットを削除します。

1. **[IP アドレス空間]** タブで **[+ サブネットの追加]** を選択します。

1. **[サブネットの追加]** ペインで、サブネット名を **myBackendSubnet** に、開始アドレスを **10.1.0.0** に、サイズを **/24** に設定します。

1. **[プライベート ネットワークを有効にする (既定の送信アクセスなし)]** がチェックされて**いない**ことを確認します。 変更内容を**保存**します。 **[追加]** を選択します

1. 再度、**[+ サブネットの追加]** を選択し、サブネット名を **myFrontEndSubnet** に、開始アドレスを **10.1.2.0** に、サイズを **/24** に設定します。 **[追加]** を選択します

1. **AzureBastionSubnet** があることを確認し、必要に応じて追加します。
...
2. Cloud Shell 画面のツールバーで、**[ファイルの管理]** アイコンを選択し、ドロップダウン メニューで **[アップロード/ダウンロード]** を選択して、azuredeploy.json と azuredeploy.parameters.json というファイルを Cloud Shell のホーム ディレクトリにアップロードします。
...
1. **[ロード バランサー]** を検索して選択します。

1. **[作成]** を選択し、ドロップダウンで **[Standard Load balancer]** を選択します。
...
1. **[フロントエンド IP 構成の追加]** を選択します。 下記の表の情報を入力し、**[保存]** を選択します。
...
1. **[負荷分散規則の追加]** ページで、次の表の情報を入力します。
   ...
   | セッション永続化    | **なし**                 |
   | アイドル タイムアウト (分) | **15**                   |
   | TCP リセットを有効にする       | **チェックなし**          |
   | フローティング IP を有効にする     | **チェックなし**          |
...
1. **[仮想マシンの作成]** ページの **[基本]** タブで、以下の表の情報を使用して最初の VM を作成します。
    ...
    > **注:**  イメージが使用できない場合は、別のイメージを選択します。 細かいことは関係なく、テスト用の VM を作成しているだけです。VM には IIS Web サーバーがインストールされている必要があります。 
   ...
   | サイズ                 | **Standard_D2s_v3 - 2 vcpu、8 GiB メモリ**   |
   | ユーザー名             | **TestUser**                                 |
