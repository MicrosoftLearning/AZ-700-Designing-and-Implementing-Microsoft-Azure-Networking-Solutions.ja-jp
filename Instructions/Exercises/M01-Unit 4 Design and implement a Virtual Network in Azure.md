...
Exercise:
  ...
  description: Azure portal で仮想ネットワークをデプロイします。
  duration: 20 minutes
  level: 300
  islab: true
  primarytopics:
...
1. 次の表の情報を使用して、CoreServicesVnet 仮想ネットワークを作成します。  
   ...
   |              | リージョン             | (米国) 米国東部         |
   | アドレス空間 | IPv4 アドレス空間 | 10.20.0.0/16         |

1. 次の表の情報を使用して、CoreServicesVnet のサブネットを作成します。

1. 各サブネットの作成を始めるには、**[+ サブネットの追加]** を選択します。 各サブネットの作成を終えるには、**[追加]** を選択します。 必要に応じて、既定のサブネットを**編集** (鉛筆アイコン) または**削除**します。
...
## タスク 3: ManufacturingVnet の仮想ネットワークとサブネットを作成する
   ...
   |              | リージョン             | (ヨーロッパ) 西ヨーロッパ |
   | アドレス空間 | IPv4 アドレス空間 | 10.30.0.0/16         |
...
## タスク 4: ResearchVnet の仮想ネットワークとサブネットを作成する
   ...
   |              | Name               | ResearchVnet         |
   |              | リージョン             | 東南アジア       |
   | アドレス空間| IPv4 アドレス空間 | 10.40.0.0/16         |
...
+ [仮想ネットワークを構成します](https://learn.microsoft.com/training/modules/configure-virtual-networks/)。 仮想ネットワークとサブネットの構成方法について、IP アドレスの指定も含め、説明します。
