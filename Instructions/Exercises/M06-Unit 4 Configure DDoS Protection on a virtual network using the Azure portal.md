...
Exercise:
  ...
  description: DDoS Protection を構成します。
  duration: 40 minutes
  level: 400
  islab: true
  primarytopics:
...
1. **[基本]** タブで、次の表の情報を使用して新しい VM を作成します。
    ...
    >**注:**  お使いのリージョンで Ubuntu イメージやサイズが利用できない場合は、別のものを選択してください。 イメージやサイズは、アラートを構成する上で重要ではありません。 
   ...
   | Image                 | **Ubuntu Server 22.04 LTS - x64 Gen 2** (必要な場合は、[VM の世代の構成] リンクを選びます) |
   | サイズ                  | **[すべてのサイズを表示]** を選び、**[D シリーズ v3]**、**[Standard_D2s_v3]** の順に選びます |
   | 認証の種類   | **SSH 公開キー**                                           |
   | ユーザー名              | **azureuser**                                                |
