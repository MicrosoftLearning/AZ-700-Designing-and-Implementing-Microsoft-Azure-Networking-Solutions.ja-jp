...
6. **[DNS 名ラベル]** に「**mypublicdnsxx**」と入力します (xx は、これを一意にするためのユーザーのイニシャルです。エラーが出る場合は数字を追加するなどして一意の名前にしてください)。
...
4. **[ネットワーク]**タブで **パブリックIP** を **なし** に設定します。

5. **[Review + create](レビュー + 作成)** をクリックします。

6. **Create** をクリックしてください。

7. **[新しいキーの組を生成]** ダイアログ ボックスで、**[Download private key and create resource](秘密キーをダウンロードしてリソースを作成する)** をクリックします。
...
5. **[アラート ルールの作成]** ページの **[スコープ]** タブで、**[範囲の選択]** をクリックします。
...
7. **[リソース]** の一覧で **MyPublicIPAddress** を選択し、**[適用]** をクリックします。

8. **[アラート ルールの作成]** ページの **[シグナル名]** で、**[Under DDoS attack or not]** を選択します。
...
12. **[詳細]** タブの **[アラート ルール名]** に「**MyDdosAlert**」と入力します。
...
1. [BreakingPoint Cloud](https://breakingpoint.cloud/) で Sign up を選択してアカウントを作成します（マイクロソフトアカウントを使用してください）

2. 下のスクリーンショットの設定に従って DDoS テストを設定しますが (DDoS プロファイルで **TCP SYN Flood を選択し、100k pps テスト サイズを選択してください)、 **[ターゲット IP アドレス]** ボックスでは自分の **MyPublicIPAddress** リソースの IP アドレスを指定します (例: **51.140.137.219**) ![DDOSAttack](https://user-images.githubusercontent.com/46939028/138599420-58bef33a-2597-4fa2-919f-bf1614037bc3.JPG)
