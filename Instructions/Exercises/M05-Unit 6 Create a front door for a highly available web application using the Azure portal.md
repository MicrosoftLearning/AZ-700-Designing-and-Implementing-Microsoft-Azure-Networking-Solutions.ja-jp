...
Exercise:
  ...
  description: Azure Front Door を作成して構成します。
  duration: 30 minutes
  level: 400
  islab: true
  primarytopics:
...
+ [Azure で HTTP(S) トラフィックを負荷分散する](https://learn.microsoft.com/training/modules/load-balancing-https-traffic-azure/)。 HTTP(S) トラフィック用のロード バランサー ソリューションを設計する方法と、Azure Application Gateway と Azure Front Door を実装する方法について説明します。
