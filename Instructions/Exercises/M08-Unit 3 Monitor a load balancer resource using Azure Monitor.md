...
Exercise:
  ...
  description: 内部ロード バランサーのパフォーマンスを確認するように Azure Monitor を構成します。
  duration: 55 minutes
  level: 400
  islab: true
  primarytopics:
...
1. **[BastionHost]** で、**[有効化]** を選択し、以下の表の情報を入力します。
    ...
    | --------------------------------- | ------------------------------------------------------ |
    | 要塞名                      | `myBastionHost`                                   |
    | パブリック IP アドレス                 | **[新規作成]** を選択して [名前] を **myBastionIP** に変更します。 |
...
1. このタスクでは、アーキテクチャ図に示されているように 2 つのサブネットを持つ仮想ネットワークを作成します。 
  
1. 通知を監視して、デプロイが成功していることを確認してください。 
...
1. **[作成]**、**[Standard Load Balancer]** の順に選択します。
...
1. **[フロントエンド IP 構成]** タブに移動し、**[+ フロントエンド IP 構成の追加]** を選択します。
...
1. 選択内容を**保存**してから、**[確認および作成]** を選択します。
...
1. Cloud Shell 画面のツールバーで、**[ファイルの管理]** アイコンを選択し、ドロップダウン メニューで **[アップロード]** を選択して、**azuredeploy.json** と **azuredeploy.parameters.json** というファイルを Cloud Shell のホーム ディレクトリにアップロードします。
    ...
    > **注:** 自分のサブスクリプションで作業している場合、[テンプレート ファイル](https://github.com/MicrosoftLearning/AZ-700-Designing-and-Implementing-Microsoft-Azure-Networking-Solutions/tree/master/Allfiles/Exercises)は GitHub ラボ リポジトリで入手できます。 必ず Lab 08 を使用してください。 

1. 次の ARM テンプレートをデプロイして、この演習に必要な仮想ネットワーク、サブネット、VM を作成します。 **注**: **管理者**パスワードを入力するように求められます。 パスワードはメモしておいてください。後のステップで必要になります。 
...
1. 次のステップに進む前に、VM がデプロイされ、実行中でなければなりません。 
...
1. **[IP 構成]** セクションで、**[追加]** を選択します。

1. 表示されているすべての仮想マシンを選択し、**[追加]** を選択します。

1. **[myVM1]** と **[myVM2]** のチェックボックスをオンにして、**[追加]** を選択します。
...
1. **[接続]** を選択してから **[Bastion 経由で接続する]** を選択します。
...
1. ロード バランサーの IP アドレス (10.1.0.4 など) を入力 (または貼り付け) します。

1. 2 つのバックエンド サーバー仮想マシン (myVM1 または myVM2) のどちらかが応答します。 さらにページを更新すると、応答がバックエンド サーバーからランダムに返されることがわかります。 
...
+ [Azure Monitor の概要](https://learn.microsoft.com/training/modules/intro-to-azure-monitor/) このモジュールでは、Azure Monitor を使用して、Azure リソースのパフォーマンスと操作に関する分析情報を提供する方法について説明します。
