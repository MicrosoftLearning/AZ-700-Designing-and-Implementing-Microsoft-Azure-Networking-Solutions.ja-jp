...
8. **[サブネットの追加]** ペインで、default サブネット名を **myBackendSubnet** に変更し、サブネット アドレス範囲を **10.1.0.0/24** に設定します。
...
3.  **[基本]** タブで、以下の表の情報を使用して、ロード バランサーを作成します。
   ...
   フロントエンド IP 構成タブで **+ フロントエンド IP 構成の追加** をクリック。
...
1. ロード バランサーの **[設定]** で、**[正常性プローブ]** をクリックし、**[追加]** をクリックします。
