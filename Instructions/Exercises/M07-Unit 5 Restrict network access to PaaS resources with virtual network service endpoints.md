...
Exercise:
  ...
  description: 仮想ネットワーク エンドポイントを使用してストレージへのネットワーク アクセスを制限します。
  duration: 35 minutes
  level: 500
  islab: true
  primarytopics:
...
1. **[IP アドレス]** タブを選択し、次の値を入力します (サブネット名を変更するには、**[既定値]** を選択します)。
...
1. 仮想ネットワークにサブネットを追加します。 次の図に示すように、**[設定]** で **[サブネット]** を選んで、**[+ サブネット]** を選びます。
...
1. Azure Storage サービスへの送信方向の通信を許可するルールを作成します。 次の情報を入力または選択します。
...
1. ストレージ アカウントの **[データ ストレージ]** ブレードで、**[従来のファイル共有]** を選択します。
...
1. ストレージ アカウントに移動し、**[従来のファイル共有]** を選択して、**[marketing]** ファイル共有を選択します。 
