...
1. Azure portal のホーム ページで、仮想ネットワークを検索し、結果から **[仮想ネットワーク]** を選択します。
...
1. **[セキュリティ]** タブを選択し、次の値を入力します。![グラフィカル ユーザーインターフェイス、テキスト、アプリケーション、電子メール 自動的に生成された説明](../media/ create-virtual-network-security.png)
...
1. 次の図に示すように、 **[ファイル共有]** を選択します。![グラフィカル ユーザー インターフェイス、アプリケーション 自動的に生成された説明](../media/new-file-share-2.png)
1. **[+ ファイル共有]** を選択します。
1. **[名前]** に「marketing」と入力し、 **[次へ: バックアップ]** を選びます。
   ![グラフィカル ユーザー インターフェイス、アプリケーション 自動的に生成された説明](../media/new-file-share-basics.png)
1. 次の図に示すように、 **[バックアップの有効化]** をオフにします。![自動的に生成されたグラフィカル ユーザー インターフェイス、アプリケーションの説明](../media/new-file-share-backup.png)
1. **[確認および作成]** を選択します。 リソースが検証された後、**[作成]** を選択します。
...
1. **[+ 既存の仮想ネットワークを追加]** を選択します。

1. **[ネットワークの追加]** で、次の値を選択します。![グラフィカル ユーザー インターフェイス、アプリケーション 自動的に生成された説明](../media/add-network-access.png)
...
1. **[Show Keys](キーの表示)** を選択します。 **[キー]** の値をメモします。後の手順でファイル共有を VM のドライブ文字にマップするときに、この値を手入力する必要があります。
...
1. [Cloud Shell] ペインのツール バーで、 **[ファイルのアップロード/ダウンロード]** アイコンを選択し、ドロップダウン メニューで **[アップロード]** を選択して、**VMs.json** ファイルと **VMs.parameters.json** ファイルを、ソース フォルダー **F:\Allfiles\Exercises\M07** から Cloud Shell のホーム ディレクトリに 1 つずつアップロードします。
...
1. [接続] ボタンと [RDP] を選択したら、[RDP ファイルのダウンロード] ボタンを選択します。 リモート デスクトップ プロトコル (.rdp) ファイルが作成され、お使いのコンピューターにダウンロードされます。
1. ダウンロードされた rdp ファイルを開きます。 メッセージが表示されたら、[接続] を選択します。 VM の作成時に指定したユーザー名とパスワードを入力します。 場合によっては、 [その他] 、 [別のアカウントを使用する] を選択して、VM の作成時に入力した資格情報を指定する必要があります。
1. **[OK]** を選択します。
1. サインイン処理中に証明書の警告が表示される場合があります。 警告を受け取ったら、[はい] または [続行] を選択して接続処理を続行します。
1. ContosoPrivate VM で、PowerShell を使用して、Azure ファイル共有を Z ドライブにマップします。 次のコマンドを実行する前に、<storage-account-key>、<storage-account-name> (つまり contosostoragexx) と、my-file-share (つまり マーケティング) を、ストレージ アカウントを作成するタスクで指定した値と置き換えます。
...
1. VM にコマンド プロンプトからインターネットへの送信接続が**ある**ことを確認します。
...
1. 検索結果に "**ContosoPublic**" が表示されたら、それを選択します。

1. 「ストレージ アカウントへのアクセスを確認する」タスクの手順 1 から 6 を、ContosoPublic VM に対して実行します。  
...
1. パブリック VM に、コマンド プロンプトからインターネットへの送信接続がないことを確認します。

 ping bing.com    

1. ContosoPublic VM へのリモート デスクトップ セッションを終了します。

1. お使いのコンピューターから、Edge を使用して Azure Portal にアクセスします。

1. **[リソース、サービス、ドキュメントの検索]** ボックスに、作成したストレージ アカウントの名前を入力します。 指定したストレージ アカウントの名前が検索結果に表示されたら、それを選びます。

1. **[ファイル共有]** を選択したら、**marketing** ファイル共有を選択します。

1. **Browse** をクリックすると、次のスクリーンショットに示すようにエラーが表示されます。
